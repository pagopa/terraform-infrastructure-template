...
# LOAD VARIABLES
#
aks_name_from_cli=$(az aks list -o tsv --query "[?contains(name,'aks')].{Name:name}")
echo "[INFO] aks_name_from_cli: ${aks_name_from_cli}"
aks_resource_group_name_from_cli=$(az aks list -o tsv --query "[?contains(name,'aks')].{Name:resourceGroup}")
